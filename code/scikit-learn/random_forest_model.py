...
class model():
    def __init__(self, current_city):
        ...
        if len(y_weather) >= 5:
            model_w = RandomForestRegressor(n_estimators=100, random_state=42).fit(X, y_weather)
        ...
        if len(y_aq) >= 5:
            model_aq = RandomForestRegressor(n_estimators=100, random_state=42).fit(X, y_aq)
