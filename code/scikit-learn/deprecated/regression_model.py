...
class regression_model():
    def __init__(self):
        ...
        # ---------------------------
        city_data = {}
        for city, files in cities_files.items():
            try:
                df_city = load_and_prepare_city(files)
                if df_city.empty:
                    print(f"Loaded {city} but resulting dataframe is empty; skipping.")
                    continue
                df_city['city'] = city
                city_data[city] = df_city
                print(f"Loaded {city}: {len(df_city)} feeling rows (date range {df_city['date'].min().date()} -> {df_city['date'].max().date()})")
        ...
        # ---------------------------
        self.models = {}
        for city, df_city in city_data.items():
            ...
            if len(y_weather) >= 5:
                model_w = LinearRegression().fit(X, y_weather)
                r2_w = r2_score(y_weather, model_w.predict(X))
                self.models[city]['weather'] = model_w
                self.models[city].setdefault('scores', {})['weather_r2'] = r2_w
                print(f"{city} - weather model trained on {len(y_weather)} rows, R^2 = {r2_w:.3f}")
            else:
                print(f"{city} - not enough samples for weather model ({len(y_weather)} rows).")

            # Air quality satisfaction model
            y_aq = df_city['air_quality_satisfaction'].values
            if len(y_aq) >= 5:
                model_aq = LinearRegression().fit(X, y_aq)
                r2_aq = r2_score(y_aq, model_aq.predict(X))
                self.models[city]['air_quality'] = model_aq
                self.models[city].setdefault('scores', {})['air_quality_r2'] = r2_aq
                print(f"{city} - air quality model trained on {len(y_aq)} rows, R^2 = {r2_aq:.3f}")
            else:
                print(f"{city} - not enough samples for air quality model ({len(y_aq)} rows).")
    ...
    def predict_feelings(self, city, forecast_temp = 0, forecast_aqi = 0):
        if city not in self.models:
            raise ValueError(f"No models available for city '{city}'. Available: {list(self.models.keys())}")
        inp = np.array([[float(forecast_temp), float(forecast_aqi)]])
        out = {}
        modelW = self.models[city].get('weather')
        out['weather_satisfaction'] = self.clip_1_10(modelW.predict(inp)[0]) if modelW is not None else None

        modelA = self.models[city].get('air_quality')
        out['air_quality_satisfaction'] = self.clip_1_10(modelA.predict(inp)[0]) if modelA is not None else None
        return out

    def runExample(self):
        for city in self.models.keys():
            try:
                print(city, self.predict_feelings(city, forecast_temp=1.0, forecast_aqi=1.0))
            except Exception as e:
                print(f"Prediction error for {city}: {e}")
    def run(self, city, forecast_temp, forecast_aqi):
        try:
            return {city: self.predict_feelings(city, forecast_temp = forecast_temp, forecast_aqi=forecast_aqi)}
        except Exception as e:
            print(f"Prediction error for {city}: {e}")
...
for city in ["Lahore", "Islamabad", "Karachi"]:
    for i in range(len(temps)):
        values = _.run(city, temps[i], aqis[i])
...
print(df)
df.to_csv("M:/Arbeit/Schule/internship/python/tests/regression_model_testA.csv")
